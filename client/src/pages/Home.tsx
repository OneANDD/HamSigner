...
function FileDropZone({ label, accept, icon, file, onFile, disabled, hint }: FileDropZoneProps) {
...
export default function Home() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // The server transitions uploading → signing internally; we show "signing" after a brief delay
      ...
      <header className="border-b border-border bg-card/50 backdrop-blur-sm sticky top-0 z-10">
        <div className="container flex items-center justify-between h-14">
          ...
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            ...
            <span>Files processed server-side &amp; deleted after signing</span>
          </div>
        </div>
      </header>
